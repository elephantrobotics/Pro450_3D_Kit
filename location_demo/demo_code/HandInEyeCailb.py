...
if __name__ == "__main__":
    ...
    erobot = Pro450Client() 
    if erobot.is_power_on()!=1:
        erobot.power_on()
    erobot.set_end_type(0)
    rvs_ip = "localhost"
    rvs_port = 2013
    conSuc_rvs, sock_rvs=connectRvsServer(rvs_ip, rvs_port)#连接RVS

    # cal_joint=[29.02, 26.2, -111.78, -7.39, -0.47, 74.42]
    cal_joint=[29.01, 20.27, -84.2, -29.04, -0.47, 74.41]
    J1 = cal_joint[0]
    J2 = cal_joint[1]
    J3 = cal_joint[2]
    J4 = cal_joint[3]
    ...
    while exec_index <len(point):
        ...
        print("移动到拍照位")
        #移动
        erobot.send_angles(point[exec_index-1], robot_speed)
        time.sleep(2)
